...
export function generateInvoicePdf(
  ...
  doc.text(`Generated by: ${currentUser}`, pageWidth - margin, yPos+5, { align: 'right' });
  yPos += 5;
  ...
  const drawTableHeaders = () => {
    ...
    doc.text('Serial Number', colStartX.snBarcode, yPos);
  ...
  order.items.forEach((item,index) => {
    ...
    if(maxLinesPerItem>1){
      yPos+=maxLinesPerItem*5;
